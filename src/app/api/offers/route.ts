...
export async function POST(req: NextRequest) {
  try {
    ...
    if (listing.status !== "ACTIVE")
      throw new ApiError("Listing is not active");
    if (listing.listingType !== "OFFER")
      throw new ApiError("This listing does not accept offers");
    if (listing.sellerId === user.sub)
      throw new ApiError("You cannot make an offer on your own listing", 403);
