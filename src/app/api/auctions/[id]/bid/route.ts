...
export async function POST(
  ...
  try {
    ...
    if (auctionCheck.sellerId === user.sub)
      throw new ApiError("Sellers cannot bid on their own auction", 403);

    // All validation + writes inside a single transaction to prevent race conditions
    const { bid, updatedAuction, previousHighBidderId, extended } =
      await prisma.$transaction(async (tx) => {
        ...
        if (auction.status !== "LIVE")
          throw new ApiError("Auction is not live");

        const now = new Date();
        if (now >= auction.endTime)
          throw new ApiError("Auction has already ended");
        ...
        const bid = await tx.bid.create({
          data: { auctionId, bidderId: user.sub, amount },
        });
    ...
    return Response.json(
      { auction: updatedAuction, bid, extended },
      { status: 201 },
    );
  } catch (err) {
